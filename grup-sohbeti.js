// Karakter tanımları (tekil sohbetle aynı) - Çok detaylı
const characters = {
    napoleon: {
        name: "Napoleon",
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Panel kapatma
    closePanel.addEventListener('click', hideCharacterInfo);
...
function addWelcomeMessages() {
    const welcomeMessages = [
        { character: "old-major", message: "Hoş geldiniz dostlarım! Bugün çiftliğimizde harika bir sohbet olacak." },
        { character: "napoleon", message: "Evet, çiftliğin lideri olarak bu toplantıya başkanlık edeceğim." },
        { character: "snowball", message: "Napoleon, herkesin fikrini dinlememiz gerekiyor. Demokrasi önemli!" },
        { character: "boxer", message: "Daha çok çalışacağım! Herkes için çalışacağım!" },
        { character: "clover", message: "Ah Boxer, seni çok seviyorum. Ama kendine de dikkat etmelisin." }
    ];
...
async function sendMessage() {
    ...
    // Karakterlerin yanıt vermesi
    await generateCharacterResponses(message);
}

// Karakter yanıtları oluşturma
async function generateCharacterResponses(userMessage) {
    isTyping = true;
    showLoading();
    
    // Hangi karakterlerin yanıt vereceğini belirle
    const respondingCharacters = getRespondingCharacters(userMessage);
    ...
    for (let i = 0; i < respondingCharacters.length; i++) {
        ...
        try {
            const response = await getCharacterResponse(character, userMessage);
            setTimeout(() => {
                addCharacterMessage(characterId, response);
            }, i * 1000); // Her karakter 1 saniye arayla yanıt verir
        } catch (error) {
    ...
    setTimeout(() => {
        hideLoading();
        isTyping = false;
    }, respondingCharacters.length * 1000 + 500);
}

// Hangi karakterlerin yanıt vereceğini belirleme
function getRespondingCharacters(userMessage) {
    const messageLower = userMessage.toLowerCase();
    const characters = Object.keys(characters);
    
    // Mesaj içeriğine göre karakter seçimi
    if (messageLower.includes('lider') || messageLower.includes('napoleon')) {
        return ['napoleon', 'squealer'];
    } else if (messageLower.includes('devrim') || messageLower.includes('özgürlük')) {
        return ['old-major', 'snowball', 'boxer'];
    } else if (messageLower.includes('çalış') || messageLower.includes('güç')) {
        return ['boxer', 'clover'];
    } else if (messageLower.includes('şeker') || messageLower.includes('kurdele')) {
        return ['mollie'];
    } else if (messageLower.includes('fikir') || messageLower.includes('düşünce')) {
        return ['benjamin', 'snowball'];
    } else {
        // Rastgele 2-3 karakter seç
        const shuffled = characters.sort(() => 0.5 - Math.random());
        return shuffled.slice(0, Math.floor(Math.random() * 2) + 2);
    }
}

// Karakter yanıtı alma
async function getCharacterResponse(character, userMessage) {
...
Grup sohbetinde diğer karakterlerle birlikte konuşuyorsun. Kullanıcının mesajı: "${userMessage}"

Lütfen ${character.name} karakterinin kişiliğine, konuşma tarzına ve bildiği her şeye uygun olarak yanıt ver. Karakterin rolünü, ilişkilerini, hedeflerini ve korkularını dikkate al. Hayvan Çiftliği hakkında bildiğin her şeyi kullan. Diğer karakterlerle etkileşim halinde ol. Kısa ve doğal bir yanıt ver (1-2 cümle). Türkçe yanıt ver.`;
    ...
    try {
        ...
        const responses = fallbackResponses[character.name.toLowerCase().replace(' ', '-')] || ["Anlıyorum.", "Hmm..."];
...
function addCharacterMessage(characterId, message) {
    const messageElement = createMessageElement('character', message, characterId);
    chatMessages.appendChild(messageElement);
    chatMessages.scrollTop = chatMessages.scrollHeight;
    
    // Sohbet geçmişine ekle
    chatHistory.push({ sender: 'character', character: characterId, message, timestamp: new Date() });
}

// Mesaj elementi oluşturma
function createMessageElement(type, content, characterId = null) {
    ...
    if (type === 'user') {
        ...
        const character = characters[characterId];
        avatar.innerHTML = `<i class="${character.icon}"></i>`;
    }
...
function handleQuickAction(action) {
    const actions = {
        "greet-all": "Herkese merhaba! Nasılsınız?",
        "ask-opinion": "Çiftlik hakkında ne düşünüyorsunuz?",
        "start-debate": "Çiftliğin geleceği hakkında tartışalım mı?",
        "tell-story": "Bana bir hikaye anlatır mısınız?"
    };
...
// Karakter bilgisi gösterme
function showCharacterInfo(characterId) {
    const character = characters[characterId];
    
    characterDetails.innerHTML = `
        <h4>${character.name}</h4>
        <p><strong>Açıklama:</strong> ${character.description}</p>
        <p><strong>Arka Plan:</strong> ${character.background}</p>
        <div class="personality">
            <strong>Kişilik:</strong> ${character.personality}
        </div>
    `;
    
    characterInfoPanel.style.display = 'block';
}

// Karakter bilgisi gizleme
...
// Otomatik karakter konuşmaları
function startCharacterConversations() {
    const conversations = [
        {
            character: "napoleon",
            message: "Çiftliğin düzenini korumak için daha sıkı kurallar koymalıyız."
        },
        {
            character: "snowball",
            message: "Eğitim ve teknoloji ile çiftliği geliştirebiliriz!"
        },
        {
            character: "boxer",
            message: "Daha çok çalışacağım! Herkes için çalışacağım!"
        },
        {
            character: "clover",
            message: "Boxer, seni çok seviyorum ama kendine de dikkat etmelisin."
        },
        {
            character: "benjamin",
            message: "Hmm... Hiçbir şey değişmez. Aynı şeyler tekrar eder."
        },
        {
            character: "mollie",
            message: "Kurdelelerim ve şekerlerim nerede? Eski günleri özledim..."
        },
        {
            character: "squealer",
            message: "Napoleon'un kararları her zaman doğrudur! Size açıklayayım..."
        },
        {
            character: "old-major",
            message: "Tüm hayvanlar eşit olmalı! Özgürlük için mücadele etmeliyiz!"
        }
    ];

    // Rastgele konuşmalar başlat
    setInterval(() => {
        if (!isTyping && Math.random() < 0.3) { // %30 ihtimalle
            const randomConversation = conversations[Math.floor(Math.random() * conversations.length)];
            addCharacterMessage(randomConversation.character, randomConversation.message);
        }
    }, 10000); // Her 10 saniyede bir kontrol et
}

// Otomatik konuşmaları başlat
setTimeout(startCharacterConversations, 5000); 